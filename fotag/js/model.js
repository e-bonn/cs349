...
function createModelModule() {
    ...
    _.extend(ImageModel.prototype, {
        ...
        setCaption: function(caption) {
            this.caption = caption;
            this.notify(IMAGE_META_DATA_CHANGED_EVENT,this);
        },
    ...
    var ImageCollectionModel = function() {
        ...
        this.notify = function(eventType, imageModelCollection, imageModel) {
            _.each(this.listeners, function(listener) {
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            var self = this;
            var imgmdllstnr = function (imageModel, eventTime){
                self.notify(IMAGE_META_DATA_CHANGED_EVENT,self,this);
            }
            this.imageModels.push(imageModel);
            this.imageModelListeners.push(imgmdllstnr);
            imageModel.addListener(imgmdllstnr);
            this.notify(IMAGE_ADDED_TO_COLLECTION_EVENT,self,imageModel);
        },
        ...
        removeImageModel: function(imageModel) {
            ...
            if (index > -1){
                imageModel.removeListener(this.imageModelListeners[index]);
                this.imageModelListeners.splice(index,1);
                this.imageModels.splice(index,1);
                this.notify(IMAGE_REMOVED_FROM_COLLECTION_EVENT,this,imageModel);
            }
        },
